...
from ..models import GroupChatRequest, GroupMessageRequest
from ..brain_registry import BRAIN_REGISTRY
from ..services.billing_service import is_subscriber
from ..services.group_chat_service import generate_synthesis, stream_group_turn
...
async def create_group_session(
    ...
    if (count.count or 0) >= daily_limit:
        raise HTTPException(status_code=429, detail=f"You've reached your limit of {daily_limit} group sessions today.")
...
@router.post("/group-chat/{session_id}/message")
...
async def group_message(
    ...
    session = await db.table("group_sessions").select("brain_slugs,status").eq("id", str(session_id)).eq("user_id", user_id).execute()
    if not session.data:
        raise HTTPException(status_code=404, detail="Session not found.")

    brain_slugs = session.data[0]["brain_slugs"]

    return StreamingResponse(
        stream_group_turn(str(session_id), user_id, brain_slugs, body.message),
        media_type="text/event-stream",
        headers={"Cache-Control": "no-cache", "X-Accel-Buffering": "no"},
    )


@router.post("/group-chat/{session_id}/synthesize")
...
async def synthesize(
    ...
    session = await db.table("group_sessions").select("id").eq("id", str(session_id)).eq("user_id", user_id).execute()
    if not session.data:
        raise HTTPException(status_code=404, detail="Session not found.")

    content = await generate_synthesis(str(session_id), user_id)
